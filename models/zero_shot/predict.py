...
def get_inference_configs():
    ...
    config.ckpt_path = "models/zero_shot/zeroshot_asp_full.ckpt" # path to zeroshot_asp_full.ckpt
    config.sed_ckpt_path = "models/zero_shot/htsat_audioset_2048d.ckpt" # path to htsat_audioset_2048d.ckpt
...
class Predictor(BasePredictor):
    ...
    def predict(self, mix_file, query_file):
        ...
        trainer.test(self.at_wrapper, dataloaders=query_loader)
        ...
        trainer.test(exp_model, dataloaders=ref_loader)

        prediction_path = os.path.join(
            self.config.wave_output_path, f"out.wav"
        )
